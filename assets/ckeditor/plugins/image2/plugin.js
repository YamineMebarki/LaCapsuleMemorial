...
( function() {
	...
	// 		│      │</wrapper>                     │                             │
	// 		├──────┼───────────────────────────────┼─────────────────────────────┤
	// 		│left  │<wrapper style=”float:left”>   │<images style=”float:left” />   │
	// 		│      │ <images />                       │                             │
	// 		│      │</wrapper>                     │                             │
	// 		├──────┼───────────────────────────────┼─────────────────────────────┤
	// 		│center│<wrapper>                      │<p style=”text-align:center”>│
	// 		│      │ <p style=”text-align:center”> │  <images />                    │
	// 		│      │   <images />                     │</p>                         │
	// 		│      │ </p>                          │                             │
	// 		│      │</wrapper>                     │                             │
	// 		├──────┼───────────────────────────────┼─────────────────────────────┤
	// 		│right │<wrapper style=”float:right”>  │<images style=”float:right” />  │
	// 		│      │ <images />                       │                             │
	// 		│      │</wrapper>                     │                             │
	// 		└──────┴───────────────────────────────┴─────────────────────────────┘
	//
	...
	// 		│      │</wrapper>                     │                             │
	// 		├──────┼───────────────────────────────┼─────────────────────────────┤
	// 		│left  │<wrapper class=”left”>         │<images class=”left” />         │
	// 		│      │ <images />                       │                             │
	// 		│      │</wrapper>                     │                             │
	// 		├──────┼───────────────────────────────┼─────────────────────────────┤
	...
	// 		│      │</wrapper>                     │                             │
	// 		├──────┼───────────────────────────────┼─────────────────────────────┤
	// 		│right │<wrapper class=”right”>        │<images class=”right” />        │
	// 		│      │ <images />                       │                             │
	// 		│      │</wrapper>                     │                             │
	// 		└──────┴───────────────────────────────┴─────────────────────────────┘
	//
	...
	function widgetDef( editor ) {
		...
		return {
			...
			requiredContent: 'images[src,alt]',

			features: getWidgetFeatures( editor ),

			styleableElements: 'images figure',

			// This widget converts style-driven dimensions to attributes.
			contentTransformations: [
				[ 'images[width]: sizeToAttribute' ]
			],
		...
		stateShifter: function( editor ) {
			...
			var stateActions = {
				...
				hasCaption:	function( shift, oldValue, newValue ) {
					...
					// Get <images/> or <a><images/></a> from widget. Note that widget element might itself
					...
					if ( shift.element.is( { img: 1, a: 1 } ) )
						...
						imageOrLink =  shift.element.findOne( 'a,images' );
					...
					if ( newValue ) {
						...
						// Use old <images/> or <a><images/></a> instead of the one from the template,
					...
					// The caption was present, but now it's to be removed.
						...
						// Unwrap <images/> or <a><images/></a> from figure.
				...
				link: function( shift, oldValue, newValue ) {
					if ( shift.changed.link ) {
						var img = shift.element.is( 'img' ) ?
							...
							// Why deflate:
							// If element is <images/>, it will be wrapped into <a>,
							// which becomes a new widget.element.
							// If element is <a><images/></a>, it will be unlinked
							// so <images/> becomes a new widget.element.
						...
						// If unlinked the image, returned element is <images>.
						if ( !newValue )
			...
			function unwrapFromCentering( element ) {
				var imageOrLink = element.findOne( 'a,images' );
			...
			// Wraps <images/> -> <a><images/></a>.
			// Returns reference to <a>.
			//
			// @param {CKEDITOR.dom.element} images
			...
			// De-wraps <a><images/></a> -> <images/>.
			// Returns the reference to <images/>
			//
	...
	// Returns a function that creates widgets from all <images> and
	...
	function upcastWidgetElement( editor ) {
		...
		return function( el, data ) {
			...
			//
			// 2. <p style="text-align:center"><images/></p>.
			//    Nothing to do here: <p> remains for styling purposes.
			//
			// 3. <div style="text-align:center"><images/></div>.
			...
			if ( isCenterWrapper( el ) ) {
				...
				// Image can be wrapped in link <a><images/></a>.
			...
			// No center wrapper has been found.
				...
				// Upcast linked image like <a><images/></a>.
	...
	function downcastWidgetElement( editor ) {
		...
		// @param {CKEDITOR.htmlParser.element} el
		return function( el ) {
			// In case of <a><images/></a>, <images/> is the element to hold
	...
	function centerWrapperChecker( editor ) {
		...
		return function( el ) {
			...
			// Only <figure> or <images /> can be first (only) child of centering wrapper,
			// regardless of its type.
			if ( !( child.name in validChildren ) )
				return false;

			// If centering wrapper is <p>, only <images /> can be the child.
			//   <p style="text-align:center"><images /></p>
			if ( el.name == 'p' ) {
				if ( !isLinkedOrStandaloneImage( child ) )
					return false;
			}
			// Centering <div> can hold <images/> or <figure>, depending on enterMode.
			else {
				...
				if ( child.name == 'figure' ) {
					...
					// Centering <div> can hold <images/> or <a><images/></a> only when enterMode
					// is ENTER_(BR|DIV).
					//   <div style="text-align:center"><images /></div>
					//   <div style="text-align:center"><a><images /></a></div>
					if ( editor.enterMode == CKEDITOR.ENTER_P )
						return false;

					// Regardless of enterMode, a child which is not <figure> must be
					// either <images/> or <a><images/></a>.
					if ( !isLinkedOrStandaloneImage( child ) )
						return false;
				}
			}
	...
	// Checks whether element is <images/> or <a><images/></a>.
	//
	...
	function getWidgetFeatures( editor ) {
		var alignClasses = editor.config.image2_alignClasses,
			features = {
				dimension: {
					requiredContent: 'images[width,height]'
				},
				align: {
					requiredContent: 'img' +
 ...
 *
 *		<images alt="My image" class="custom-center-class" src="foo.png" />
 *
 * instead of:
 *
 *		<images alt="My image" style="float:left" src="foo.png" />
 *
